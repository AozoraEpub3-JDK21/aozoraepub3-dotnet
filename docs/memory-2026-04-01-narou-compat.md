...
- 新 master_68（128章 → 126話対応版に再生成）での比較結果
  - キャッシュ更新（127 → 128ファイル）後の再比較
  - 66: `total(norm) 1048`（据え置き）
  - 68: `total(norm) 2`（新マスター対比の安定ベースライン）

## 2026-04-02 追記（セッション4: master 同期確認）
- master_66 落とし直し（narou download --force）+ master_68 再整合
  - master_68 の問題: narou.rb ローカルに第127話がキャッシュされていたが live サイトリストから除外済みだった
    - 第127話 URL: `2912051596725521964` → kakuyomu `__NEXT_DATA__` に未掲載
    - 対処: narou.rb の raw/本文/toc.yaml から第127話を削除し 126 話で再変換
  - 比較結果（cache compare, 現在のベースライン）
    - 66: `total(norm) 989`（master 更新により live 差分が 1048 → 989 に削減）
    - 68: `total(norm) 2`（変わらず。安定ベースライン維持）
- 今回採用した修正
